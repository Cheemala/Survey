...
public class UserSurveyAdapter extends RecyclerView.Adapter<UserSurveyAdapter.MyViewHolder>{

    private Context context;
    private ArrayList<AdminSurveyData> surveyData;
    private SurveyResultCallbacks srvyRsltCallbacks;

    public UserSurveyAdapter(Context context, ArrayList<AdminSurveyData> surveyData, SurveyResultCallbacks srvyRsltCallbacks) {
    ...
    @Override
    public UserSurveyAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View surveyItem = LayoutInflater.from(parent.getContext()).inflate(R.layout.admin_survey_item,parent,false);
        return new UserSurveyAdapter.MyViewHolder(surveyItem);
    }

    @Override
    public void onBindViewHolder(UserSurveyAdapter.MyViewHolder holder, final int position) {
        ...
        holder.getSurveyRsltStsBtn().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(surveyData.get(position).getSurveyStatus().contentEquals("Released")){
                    srvyRsltCallbacks.getSurveyResult(surveyData.get(position).getSurveyId());
                }else {
                    Toast.makeText(context,"Survey is pending!",Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
